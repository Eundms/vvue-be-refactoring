...
public class PlaceServiceImpl implements PlaceService {
	...
	@Override
	public RecommendPlaceListResDto getRecommendPlaces(Long userId, double lat, double lng, Long distance, Long cursor,
		Long size) {
		PlaceStats placeStats = placeStatsRepository.findById(cursor)
			.orElse(PlaceStats.builder().id(-1L).avgRating(5).build());
		List<RecommendPlaceResDto> recommendPlaceReqDtoList = placeStatsRepository.findRecommendPlacesByLocation(userId,
