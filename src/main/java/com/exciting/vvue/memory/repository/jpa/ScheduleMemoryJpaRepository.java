...
public interface ScheduleMemoryJpaRepository extends JpaRepository<ScheduleMemory, Long> {
	...
	@Query("SELECT sm FROM ScheduleMemory sm " +
		...
		"ORDER BY sm.id ASC")
	List<ScheduleMemory> findByMarriedIdWithCursor(Long marriedId, Long firstScheduleMemoryId, Pageable pageable);
	...
	Long countByMarriedIdAndIdGreaterThan(@Param("marriedId") Long marriedId, @Param("id") Long lastCursorId);
}
