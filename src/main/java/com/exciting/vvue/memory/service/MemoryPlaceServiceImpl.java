...
public class MemoryPlaceServiceImpl implements MemoryPlaceService {
	...
	@Override
	public List<PlaceMemoryToMapDto> getMemoriablePlaceByMarriedId(Long marriedId, MemoryPlaceFindDto findCondition) {
				...
				List<MemoryAtPlaceDto> memoryAtPlaceDtos = groupedByScheduleId.entrySet().stream()
					.map(scheduleEntry -> {
						...
						List<PictureDto> placePictures = scheduleMemoryList.stream()
							...
							.map(image -> new PictureDto(image.getPicture().getId(), image.getPicture().getUrl()))
		...
		if (findCondition != null && findCondition.getX() != null && findCondition.getY() != null) {
			...
			placeMemoryToMapDtos.sort(Comparator.comparing(dto
				-> calculateHaversineDistance(x, y, new BigDecimal(dto.getPlace().getX()), new BigDecimal(dto.getPlace().getY()))
			));
		}
		return placeMemoryToMapDtos;
	}
	public double calculateHaversineDistance(BigDecimal x, BigDecimal y, BigDecimal placeLat, BigDecimal placeLon) {
