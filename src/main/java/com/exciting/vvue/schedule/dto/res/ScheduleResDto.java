...
public class ScheduleResDto {
  ...
  @Builder
  public ScheduleResDto(Long id, Long marriedId, String scheduleDate, String scheduleName,
  ...
  public ScheduleResDto setCommingDate() {
    LocalDate origin = LocalDate.parse(this.scheduleDate,
        DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    LocalDate scheduleLocalDate = LocalDate.parse(this.scheduleDate,
        DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    switch (this.getRepeatCycle()) {
      ...
      case MONTHLY -> {
        ...
        do {
          curLocalDate = scheduleLocalDate.plusMonths(Math.max(
              Period.between(scheduleLocalDate, LocalDate.now().minusDays(1)).toTotalMonths() - (
                  scheduleLocalDate.isBefore(LocalDate.now().minusDays(1)) ? 0 : 1) + i++, 0));
      ...
      case YEARLY -> {
        ...
        LocalDate curLocalDate;
        do {
          curLocalDate = scheduleLocalDate.plusYears(Math.max(
              Period.between(scheduleLocalDate, LocalDate.now().minusDays(1)).getYears() - (
                  scheduleLocalDate.isBefore(LocalDate.now().minusDays(1)) ? 0 : 1) + i++, 0));
        } while (curLocalDate.getDayOfMonth() != scheduleLocalDate.getDayOfMonth());
        curDate = curLocalDate.toString();
      }
